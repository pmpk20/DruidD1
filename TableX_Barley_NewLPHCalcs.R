...
Years <-
  Barley[3:51] %>% colnames() %>% gsub(pattern = "X", replacement = "") %>% as.numeric()
...
for (i in 1:length(Column_NE)) {
  ...
  ## Multiply by 10 to transform to Aphid Days
  ...
  LossPerHectare_NS <-
    (Column_Yield * Column_Price) *  YieldLoss_Truncated_NS
  ...
  ## for every possibility
  LossPerHectare_AS <-
    (Column_Yield * Column_Price)  %>% ## per hectare value
  ...
  YieldLoss_ET <-
    ifelse(Dist >= (Threshold*10), 0, Dist) %>%  
    multiply_by(12.70) %>% 
    raise_to_power(0.65) %>% 
    divide_by(100) %>% 
    ifelse(is.infinite(.), 0, .) %>% 
  ...
  Estimates[i, 2] <-
    (qnorm(p = 0.975) * (Dist %>% sd()) / sqrt(SampleSize))
  
  
  Estimates[i, 3] <- YieldLoss_Truncated_NS %>% mean(na.rm = TRUE)
  Estimates[i, 4] <-
    (qnorm(p = 0.975) * (YieldLoss_Truncated_NS %>% sd()) / sqrt(SampleSize))
  
  Estimates[i, 5] <- LossPerHectare_NS %>% mean(na.rm = TRUE)
  Estimates[i, 6] <-
    (qnorm(p = 0.975) * (LossPerHectare_NS %>% sd()) / sqrt(SampleSize))
  
  Estimates[i, 7] <- LossTotal_NS %>% mean(na.rm = TRUE)
  Estimates[i, 8] <-
    (qnorm(p = 0.975) * (LossTotal_NS %>% sd()) / sqrt(SampleSize))
  
  Estimates[i, 9] <- YieldLoss_ET %>% mean(na.rm = TRUE)
  Estimates[i, 10] <-
    (qnorm(p = 0.975) * (YieldLoss_ET %>% sd()) / sqrt(SampleSize))
  
  Estimates[i, 11] <- LossPerHectare_ET %>% mean(na.rm = TRUE)
  Estimates[i, 12] <-
    (qnorm(p = 0.975) * (LossPerHectare_ET %>% sd()) / sqrt(SampleSize))
  
  Estimates[i, 13] <- LossTotal_ET %>% mean(na.rm = TRUE)
  Estimates[i, 14] <-
    (qnorm(p = 0.975) * (LossTotal_ET %>% sd()) / sqrt(SampleSize))
  ...
  Estimates[i, 18] <-
    (qnorm(p = 0.975) * (LossPerHectare_AS %>% sd()) / sqrt(SampleSize)) %>% is.na %>% ifelse(0, 1)
  ...
  Estimates[i, 22] <-
    (qnorm(p = 0.975) * (YieldLoss_Total %>% sd()) / sqrt(SampleSize))
  
  Estimates[i, 23] <- LPH_Total %>% mean(na.rm = TRUE)
  Estimates[i, 24] <-
    (qnorm(p = 0.975) * (LPH_Total %>% sd()) / sqrt(SampleSize))
  
  Estimates[i, 25] <- LossTotal_Total %>% mean(na.rm = TRUE)
  Estimates[i, 26] <-
    (qnorm(p = 0.975) * (LossTotal_Total %>% sd()) / sqrt(SampleSize))
