...
# Summarise_LT_PCT <- function(Data, New, Old) {
#   
#   Input <- ((Data[, New] - Data[, Old])/Data[, Old]) %>% 
#     summarise_all(list(~ str_c(
#       mean(.) %>% round(2) %>% sprintf("%.2f",. ),
...
Annual_YL_Mean <- matrix(0, nrow = ColLength, ncol = 11) %>% data.frame()
Annual_YL_SD <- matrix(0, nrow = ColLength, ncol = 11) %>% data.frame()

Annual_LPH <- matrix(0, nrow = ColLength, ncol = 11) %>% data.frame()
Annual_LPH_Raw <- matrix(0, nrow = ColLength, ncol = 11) %>% data.frame()
Annual_LPH_Mean <- matrix(0, nrow = ColLength, ncol = 11) %>% data.frame()
Annual_LPH_SD <- matrix(0, nrow = ColLength, ncol = 11) %>% data.frame()


Annual_LT <- matrix(0, nrow = ColLength, ncol = 11) %>% data.frame()
Annual_LT_Raw <- matrix(0, nrow = ColLength, ncol = 11) %>% data.frame()
Annual_LT_Mean <- matrix(0, nrow = ColLength, ncol = 11) %>% data.frame()
Annual_LT_SD <- matrix(0, nrow = ColLength, ncol = 11) %>% data.frame()
...
for (Year in 1:length(Variable_Years)) {
  ...
  Annual_YL_Mean[, Year] <- rbind(
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 6, 1),
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 6, 2),
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 6, 3),
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 6, 4),
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 6, 5),
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 6, 6),
    
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 12, 7),
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 12, 8),
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 12, 9),
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 12, 10),
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 12, 11),
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 12, 12),
    
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 18, 13),
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 18, 14),
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 18, 15),
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 18, 16),
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 18, 17),
    Summarise_LPH_PCT(YieldLosss_Total_RawData, 18, 18)
  )
  ...
  Annual_LT_Mean[, Year] <- rbind(
    Summarise_LPH_PCT(LossTotal_Total_RawData, 6, 1),
    Summarise_LPH_PCT(LossTotal_Total_RawData, 6, 2),
    Summarise_LPH_PCT(LossTotal_Total_RawData, 6, 3),
    Summarise_LPH_PCT(LossTotal_Total_RawData, 6, 4),
    Summarise_LPH_PCT(LossTotal_Total_RawData, 6, 5),
    Summarise_LPH_PCT(LossTotal_Total_RawData, 6, 6),
    
    Summarise_LPH_PCT(LossTotal_Total_RawData, 12, 7),
    Summarise_LPH_PCT(LossTotal_Total_RawData, 12, 8),
    Summarise_LPH_PCT(LossTotal_Total_RawData, 12, 9),
    Summarise_LPH_PCT(LossTotal_Total_RawData, 12, 10),
    Summarise_LPH_PCT(LossTotal_Total_RawData, 12, 11),
    Summarise_LPH_PCT(LossTotal_Total_RawData, 12, 12),
    
    Summarise_LPH_PCT(LossTotal_Total_RawData, 18, 13),
    Summarise_LPH_PCT(LossTotal_Total_RawData, 18, 14),
    Summarise_LPH_PCT(LossTotal_Total_RawData, 18, 15),
    Summarise_LPH_PCT(LossTotal_Total_RawData, 18, 16),
    Summarise_LPH_PCT(LossTotal_Total_RawData, 18, 17),
    Summarise_LPH_PCT(LossTotal_Total_RawData, 18, 18)
  )
  
  # Annual_LPH_Mean[, Year] <- LossPerHectare_Total_RawData %>% as.matrix() %>% colmeans()
  # Annual_LPH_SD[, Year] <- LossPerHectare_Total_RawData %>% as.matrix() %>% colVars(std = TRUE)
  # 
  # Annual_LT_Mean[, Year] <- LossTotal_Total_RawData %>% as.matrix() %>% colmeans()
  # Annual_LT_SD[, Year] <- LossTotal_Total_RawData %>% as.matrix() %>% colVars(std = TRUE)
  # 
...
Column_LPH_Total <- Annual_LPH_Raw %>% as.matrix() %>% rowsums() %>% 
  round(2) %>% 
  format(nsmall = 2,
         big.mark = ",",
         zero.print = TRUE) %>%
