    ...
    PlotData_CLPH <- data.frame(
      ...
      dplyr::mutate(
        Insecticide = case_when(
          Insecticide == '46.69' ~ "25%",
          Insecticide == '93.38' ~ "50%", 
          Insecticide == '186.76' ~ "100%", 
          Insecticide == '280.14' ~ "150%",
          Insecticide == '373.53' ~ "200%" 
        ))
...
## Define here
Insecticides <- Insecticide_Only_PH %>% 
  mean() %>% 
  multiply_by(thresholds) %>% 
  round(2)
...
for (i in Insecticides) {
  
  # Call the Loopy() function with the given insecticides
  output <- Loopy(Insecticide_Scalar = i)
...
LPH_Plot <- PlotData_LPH %>% 
  ...
  ggtitle("Barley: Losses per hectare") +
  theme(
