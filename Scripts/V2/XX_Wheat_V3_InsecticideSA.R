...
Loopy <-
  function(Threshold_Input,
    ...
    Dist <-
      matrix(nrow = 1000, ncol = 33, data = 0) %>% as.data.frame()
    ...
    for (Year in seq_along(Variable_Years)) {
      for (i in seq_along(Column_NE)) {
        ...
        # Calculate expected economic loss with spraying
    ...
    #       Insecticide == '63.95' ~ "25%",
    #       Insecticide == '127.9' ~ "50%",
    #       Insecticide == '255.81' ~ "100%",
    #       Insecticide == '383.71' ~ "150%",
    #       Insecticide == '511.62' ~ "200%"
    #     ))
...
PlotData_CLPH %>%
  dplyr::mutate(
    Insecticide = case_when(
      Insecticide == 71.97 ~ "0.25",
      Insecticide == 143.95 ~ "0.5",
      Insecticide == 287.89 ~ "1",
      Insecticide == 431.84 ~ "1.5",
      Insecticide == 575.78 ~ "2.0",
...
LPH_Plot <- PlotData_LPH %>%
  ...
  scale_y_continuous(name = "Mean lost yield\nper hectare ",
                     breaks = seq.int(from = 0, to = 300, by = 25),
