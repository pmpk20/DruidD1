...
# Function: To do sensitivity analysis with Barley data
...
## Import all crops then drop all not barley
Data <-
  here("MarketData", "AllCropData_V2.xlsx") %>%
...
Barley_Trimmed_DF <-
  as.data.frame(x = t(Barley_Trimmed[, 3:13]), stringsAsFactors = FALSE)
...
Barley_Trimmed_DF <- Barley_Trimmed_DF %>%
  mutate(across(
    c(
      Area,
      Yield,
...
Sprays <- Barley_Trimmed_DF$MaltingSpraysCostsReal
Sprays_Insecticides <- Barley_Trimmed_DF$InsecticidesOnlyReal
...
Summarise_LPH_All_Percent <- function(Data, New, Old) {
  # Calculate the difference, handling potential division by zero
  difference <- ((Data[, New] - Data[, Old]) / 
                   ifelse(Data[, Old] == 0, NA, Data[, Old])) %>% 
    magrittr::multiply_by(100) %>% 
    as.matrix()
  
  # Remove NA values before calculating summary statistics
...
REA_Old <- 0.4749263 
REA_New <-
  (((Treatment_Distribution - Control_Distribution) / 
      Control_Distribution
  )) %>% abs()
...
Annual_LPH_Raw_ChangeMeans_NS_Percent <-
  matrix(0, nrow = ColLength, ncol = YearLength) %>% data.frame()
Annual_LPH_Raw_ChangeSDs_NS_Percent <-
  matrix(0, nrow = ColLength, ncol = YearLength) %>% data.frame()
Annual_LPH_Raw_ChangeMeans_AS_Percent <-
  matrix(0, nrow = ColLength, ncol = YearLength) %>% data.frame()
Annual_LPH_Raw_ChangeSDs_AS_Percent <-
  matrix(0, nrow = ColLength, ncol = YearLength) %>% data.frame()
Annual_LPH_Raw_ChangeMeans_ET_Percent <-
  matrix(0, nrow = ColLength, ncol = YearLength) %>% data.frame()
Annual_LPH_Raw_ChangeSDs_ET_Percent <-
  matrix(0, nrow = ColLength, ncol = YearLength) %>% data.frame()
...
Loopy <-
  function(Threshold_Input,
    ...
    for (Year in seq_along(Variable_Years)) {
      for (i in seq_along(Column_NE)) {
        ...
        if (missing(Insecticide_Scalar)) {
          IOCosts <- Insecticide_PH[Year]
        } else {
          IOCosts <- Insecticide_Scalar[Year]
        }
        # IOCosts <- ScaledInsecticides[, match(1, thresholds)][Year]
        ...
        ## THis returns KG/HA
        YL_New <- Dist[, i]  %>%
          magrittr::multiply_by(12.70) %>% 
          magrittr::raise_to_power(0.65) 
        
        ## What is the actual yield
        YL_Old <- Variable_Yield[Year]*1000 ## This was TONNES/HA now scaled to KG
        
        ## What is the difference between actual and affected
        YL_Change <- YL_Old - YL_New
        
        ## Now percentage change in lost yield
        YieldLoss_NS <- ((YL_Old - YL_Change) / YL_Change) %>%
          multiply_by(100)
        ...
        ## Value of LOSS PER HECTARE UNDER NEVER-SPRAY CONDITIONS
        LossPerHectare_NS <-
          (Variable_Yield[Year] * Variable_Price[Year]) %>%
          multiply_by(YieldLoss_NS )
        ...
        YL_Old <- Variable_Yield[Year] * 1000
        ...
        YieldLoss_ET <-
          ((YL_Old - YL_Change) / YL_Change) %>% multiply_by(100)
        ...
        ## Yield loss * expected value if not sprays
        ExpectedLossWithoutSpray <- (YieldLoss_ET )  * 
          (Variable_Yield[Year] * Variable_Price[Year])
        
        
        # Calculate expected economic loss with spraying i.e., no yield lost
        ExpectedLossWithSpray <- IOCosts
        
        # Choose the option with the least expected loss
        LossPerHectare_ET <- ifelse(
          ExpectedLossWithoutSpray < ExpectedLossWithSpray,
          ExpectedLossWithoutSpray,  
          ExpectedLossWithSpray
        )
        ...
        # LossPerHectare_ET <- ifelse(
        #   YieldLoss_ET == 0, ## if above threshold
        #   Variable_Sprays[Year], ## then you pay for normal sprays
      ...
      ## Create a vector to loop through
      ## The values in this DF are those that the function uses
      Looper <- data.frame("Left" = rep(c(11, 22, 33),
                                        each = 11) ,
                           "Right" = seq.int(from = 1,
                                             to = 33,
                                             by = 1))
      ...
      Looper_Percent <- data.frame("Left" = rep(c(11, 22, 33), each = 11) ,
                                   "Right" = seq.int(from = 1, to = 33, by = 1))
      
      
      LPH_Results_Percent <- lapply(seq_len(nrow(Looper_Percent)), function(Row) {
        Output_NS_Percent <- Summarise_LPH_All_Percent(LossPerHectare_NS_RawData,
                                                       Looper_Percent[Row, 1],
                                                       Looper_Percent[Row, 2])
        Output_ET_Percent <- Summarise_LPH_All_Percent(LossPerHectare_ET_RawData,
                                                       Looper_Percent[Row, 1],
                                                       Looper_Percent[Row, 2])
        Output_AS_Percent <- Summarise_LPH_All_Percent(LossPerHectare_AS_RawData,
                                                       Looper_Percent[Row, 1],
                                                       Looper_Percent[Row, 2])
        
        ## Return a named list
        NS_Percent <- c(
          Mean = as.numeric(Output_NS_Percent["Mean"]),
          SD = as.numeric(Output_NS_Percent["SD"])
        )
        ET_Percent <- c(
          Mean = as.numeric(Output_ET_Percent["Mean"]),
          SD = as.numeric(Output_ET_Percent["SD"])
        )
        AS_Percent <- c(
          Mean = as.numeric(Output_AS_Percent["Mean"]),
          SD = as.numeric(Output_AS_Percent["SD"])
        )
        c(NS_Percent, 
          ET_Percent,
          AS_Percent)
      ...
      Results_DF_Percent <- do.call(rbind, LPH_Results_Percent)
      
      
      Annual_LPH_Raw_ChangeMeans_NS_Percent[, Year] <-
        Results_DF_Percent[, 1] %>% as.numeric()
      Annual_LPH_Raw_ChangeSDs_NS_Percent[, Year] <-
        Results_DF_Percent[, 2] %>% as.numeric()
      
      ## We need this for plot CLPH
      Annual_LPH_Raw_ChangeMeans_ET_Percent[, Year] <-
        Results_DF_Percent[, 3] %>% as.numeric()
      Annual_LPH_Raw_ChangeSDs_ET_Percent[, Year] <-
        Results_DF_Percent[, 4] %>% as.numeric()
      
      
      ## Always spray
      Annual_LPH_Raw_ChangeMeans_AS_Percent[, Year] <-
        Results_DF_Percent[, 5]  %>% as.numeric()
      Annual_LPH_Raw_ChangeSDs_AS_Percent[, Year] <-
        Results_DF_Percent[, 6] %>% as.numeric()
    ...
    PlotData_CLPH <- data.frame(
      ...
      Pivoter() %>%  
      mutate(
        Ymin = pmin(Means - SD, 0),
        Ymax = pmin(Means + SD, 0),
        Threshold = Threshold,
        Insecticide = IOCosts)
    
    
    PlotData_CLPH_Percent <- data.frame(
      "Density" = Column_Density,
      "NE" = Column_NE,
      "Crop" = "Barley",
      
      "NS_Means" = Annual_LPH_Raw_ChangeMeans_NS_Percent %>% as.matrix() %>% rowmeans(),
      "NS_SD" = Annual_LPH_Raw_ChangeSDs_NS_Percent %>% as.matrix() %>% rowmeans(),
      
      "ET_Means" = Annual_LPH_Raw_ChangeMeans_ET_Percent %>% as.matrix() %>% rowmeans(),
      "ET_SD" = Annual_LPH_Raw_ChangeSDs_ET_Percent %>% as.matrix() %>% rowmeans(),
      
      "AS_Means" = Annual_LPH_Raw_ChangeMeans_AS_Percent %>% as.matrix() %>% rowmeans(),
      "AS_SD" = Annual_LPH_Raw_ChangeSDs_AS_Percent %>% as.matrix() %>% rowmeans()) %>%
      Pivoter() %>%  
      mutate(
        Ymin = pmin(Means - SD, 0),
        Ymax = pmin(Means + SD, 0),
    ...
    return(
      list(PlotData_CLPH,
        ...
        setNames(c(
          ...
          sprintf("PlotData_CLPH_Percent_Threshold%2s", 
                  paste(Threshold, IOCosts %>% round(0), sep = "_Insecticide")),
...
thresholds <- c(0.25, 0.5, 1, 1.5, 2)

# Create a matrix to store scaled insecticide costs
ScaledInsecticides <-
  matrix(nrow = length(Insecticide_PH),
         ncol = length(thresholds))

# Iterate over each year and threshold
for (i in seq_along(Insecticide_PH)) {
  for (j in seq_along(thresholds)) {
    ScaledInsecticides[i, j] <- round(Insecticide_PH[i] * thresholds[j], 2)
  }
}
...
for (i in thresholds) {
  ...
  # Call the Loopy() function with the given insecticides
  output <- Loopy(Insecticide_Scalar = 
                    ScaledInsecticides[, match(i, thresholds)])
...
## mutate() insecticide here for ease of merging later
PlotData_CLPH %>%
  dplyr::mutate(
    Insecticide = case_when(
      Insecticide == 53.16 ~ "0.25",
      Insecticide == 106.32 ~ "0.5",
      Insecticide == 212.65 ~ "1",
      Insecticide == 318.97 ~ "1.5",
      Insecticide == 425.3 ~ "2.0",
      TRUE ~ as.character(Insecticide)  # Handle other values if needed
    )) %>% 
  fwrite(sep = ",",
...
LPH_Plot <- PlotData_LPH %>%
  ...
  scale_y_continuous(name = "Mean lost yield\nper hectare ",
                     breaks = seq.int(from = 0, to = 400, by = 50),
