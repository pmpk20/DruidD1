  ...
  function(Threshold_Input,
    ...
    for (Year in seq_along(Variable_Years)) {
      for (i in seq_along(Column_NE)) {
        ...
        YL_Old <- Variable_Yield[Year]*1000 ## This was TONNES/HA now sclaed to KG
...
## Run with default thresholds and insecticides
YieldResults <- Loopy()
...
## Append crop for ease of combining later
Output$Crop <- "Barley"
...
YL_Plot <- YieldResults$PlotData_YL_Threshold5_Insecticide204 %>% 
  ...
  #   breaks = seq.int(from = 0, to = 6, by = 0.5),
...
## Plot Yield without AS
CLPH_Percent_Plot <-
  YieldResults$PlotData_CLPH_Percent_Threshold5_Insecticide204 %>% 
  dplyr::filter(Type != "AS") %>% 
  ggplot(aes(x = NE %>% as.numeric(),
             group = Density %>% as.factor())) +
