...
# Function: To do sensitivity analysis with wheat data
...
Wheat_Trimmed_DF <- Wheat_Trimmed_DF %>%
  mutate(across(
    c(
      Area,
      Yield,
...
Sprays <- Wheat_Trimmed_DF$MillingSprayCostsReal
Sprays_Insecticides <- Wheat_Trimmed_DF$InsecticidesOnlyReal
...
## Above but as percentage change
  ...
  function(Threshold_Input,
    ...
    for (Year in seq_along(Variable_Years)) {
      for (i in seq_along(Column_NE)) {
        ...
        LossPerHectare_NS <-
          (Variable_Yield[Year] * Variable_Price[Year]) *  (YieldLoss_NS/100)
...
CLPH_Percent_Plot <-
  YieldResults$PlotData_CLPH_Percent_Threshold5_Insecticide279 %>% 
